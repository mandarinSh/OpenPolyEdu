...
def calculate_users_and_ids(connection):
    ...
    get_unique_users_query = '''select userAndIDs.user_name as user_name, uniqueUserIds.user_id as user_id, events_count.e_count from (
            select 
		...
		ON userAndIDs.user_id = events_count.user_id
		order by user_name;'''
...
def write_result_to_file(result_file, result):
    ...
    with open(result_file,"w") as file:
        file.write(tabulate(result, headers=['user_name', 'user_id', 'count_events']))
